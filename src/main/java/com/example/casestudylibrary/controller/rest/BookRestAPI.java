...
public class BookRestAPI {
    ...
    @GetMapping
    public ResponseEntity<?> findAllWithSearch(@RequestParam(required = false, defaultValue = "") String search, @RequestParam(required = false) Long categoryId, @PageableDefault(sort = "createdAt", direction = Sort.Direction.DESC) Pageable pageable) {
    ...
    @GetMapping("/filters")
    public ResponseEntity<?> findBookByKwAndPageable(@RequestParam(required = false) String kw, @RequestParam(required = false) Long categoryId, @PageableDefault(page = 0, size = 5) Pageable pageable){
        Page<BookResDto> bookPages = bookService.findAllWithSearch(kw, categoryId, pageable);
